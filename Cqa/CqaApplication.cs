...
{
    public class CqaApplication
    {
        public CqaApplication(string projectName, string deploymentName, string endpointKey, string endpoint)
            : this((projectName, deploymentName, endpointKey, endpoint))
        {
        }

        private CqaApplication(ValueTuple<string, string, string, string> props)
        {
            ...
            {
                throw new ArgumentException($"\"{endpointKey}\" is not a valid CQA subscription key.");
            }

            if (!Uri.IsWellFormedUriString(endpoint, UriKind.Absolute))
            {
                throw new ArgumentException($"\"{endpoint}\" is not a valid CQA endpoint.");
            }
